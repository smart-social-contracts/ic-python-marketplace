#!/bin/bash
# Deploy marketplace canisters to staging network
# Usage: ./scripts/deploy_staging.sh [--reinstall]
...
echo "🚀 Deploying Marketplace to staging network..."
...
if [ "$REINSTALL_MODE" = true ]; then
    ...
    dfx deploy marketplace_backend --network staging --no-wallet --yes --mode reinstall
    dfx deploy marketplace_frontend --network staging --no-wallet --yes --mode reinstall
else
...
# Get canister IDs
BACKEND_ID=$(dfx canister id marketplace_backend --network staging)
FRONTEND_ID=$(dfx canister id marketplace_frontend --network staging)

echo ""
echo "✅ Marketplace Deployment to staging successful!"
echo ""
...
dfx canister call marketplace_backend get_name --network staging
dfx canister call marketplace_backend is_test_mode --network staging
